...
import com.xiaojihua.utils.file.PoiUtil;

/**
 * 购销合同打印，导出的excel可以直接打印
 * 这个类里面写了如何使用模版，以及设置图片区域，横线区域，函数区域，金额格式等等
 * 其中有一个细节是每个列的宽度是通过模版来设置的，所以
 * 模版里面是空的只是每个列的宽度不一样。
 * 正常设置宽度的话是需要*256那样的（C06POITest.java里面的test2）
 */
public class ContractPrint{
	public void print(Contract contract,String path, HttpServletResponse response) throws Exception{
		//相同厂家的信息一起打印
		Set<ContractProducts> oSet = contract.getContractProducts();
		List<ContractProducts> oList = new ArrayList<>(oSet);
		...
		ContractProducts oProduct = null;
		String stars = "";
		...
		String oldFactory = "";
        Iterator<ContractProducts> it = oList.iterator();
...
//		}
		...
		//这个模板里面只是设置了各个列的宽度，其他的都手工设置。
		//因为设置列的宽度比较费劲需要*256那样的公式（C06POITest.java里面的test2)
